...
watch(() => interactionStore.selectedId, async (newId) => {
  ...
  if (newId) {
    ...
    const dims = [
        { val: size.x, axis: 'x', pos: new THREE.Vector3(center.x, boundingBox.min.y, boundingBox.min.z) },
        { val: size.y, axis: 'y', pos: new THREE.Vector3(boundingBox.min.x, center.y, boundingBox.min.z) },
        { val: size.z, axis: 'z', pos: new THREE.Vector3(boundingBox.min.x, boundingBox.min.y, center.z) },
    ];
    
    dims.forEach(dim => {
        if (dim.val > 0.01) { // Only show dimensions that are not tiny
